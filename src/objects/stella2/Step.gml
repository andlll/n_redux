...
if (__b__) {
    action_move_to(pu1.x + 100 * global.sca, pu1.y - 100 * global.sca);
} else {
...
if (__b__) {
    ...
    if (__b__) {
        ...
        if (__b__) {
            action_sprite_set(sta2s, 0, 1);
        } else {
            action_sprite_set(sta2, 0, 1);
        }
    }
} else {
    action_sprite_set(sta2x, 0, 1);
}
__b__ = action_if_variable(unlocinque, 1, 0);
if (__b__) {
    __b__ = action_if_number(516, 0, 2);
    if (__b__) {
        with (r12) {
            selec = 0;
        }
